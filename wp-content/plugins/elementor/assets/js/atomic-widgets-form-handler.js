...
//#endregion
//#region modules/atomic-widgets/assets/js/frontend/utils.js
	var require_utils = /* @__PURE__ */ __commonJSMin(((exports) => {
		...
		exports.ATOMIC_FORM_SELECTOR = "[data-element_type=\"e-form\"]";
		exports.ATOMIC_FORM_FIELD_SELECTOR = "input[data-interaction-id], textarea[data-interaction-id], select[data-interaction-id]";
		exports.LINK_ACTIONS_EDITOR_WHITELIST = ["off_canvas", "lightbox"];
		exports.WHITELIST_FILTER = "frontend/handlers/atomic-widgets/link-actions-whitelist";
		var ACTION_LINK_SELECTOR = exports.ACTION_LINK_SELECTOR = "[data-action-link]";
		exports.REGISTRATION_SELECTOR = `${ACTION_LINK_SELECTOR}, :has(> ${ACTION_LINK_SELECTOR})`;
		var ELEMENTOR_DOCUMENT_SELECTOR = "[data-elementor-id]";
		...
		function getPostId(element) {
			...
			var _elementorFrontend;
			const innerDocumentId = element === null || element === void 0 || (_element$closest = element.closest) === null || _element$closest === void 0 || (_element$closest = _element$closest.call(element, ELEMENTOR_DOCUMENT_SELECTOR)) === null || _element$closest === void 0 || (_element$closest = _element$closest.dataset) === null || _element$closest === void 0 ? void 0 : _element$closest.elementorId;
			const ownerDocument = (_elementorFrontend = elementorFrontend) === null || _elementorFrontend === void 0 || (_elementorFrontend = _elementorFrontend.config) === null || _elementorFrontend === void 0 || (_elementorFrontend = _elementorFrontend.post) === null || _elementorFrontend === void 0 ? void 0 : _elementorFrontend.id;
...
//#endregion
//#region modules/atomic-widgets/assets/js/frontend/form-handlers.js
	...
	var webmcpFormListenersRegistered = false;
	...
	function registerWebmcpFormListeners() {
		if (webmcpFormListenersRegistered) return;
	...
	function resetWebmcpFormState(toolName) {
		document.querySelectorAll(_utils.ATOMIC_FORM_SELECTOR).forEach((form) => {
			if (form.getAttribute("toolname") !== toolName) return;
	...
	function extractAgentToolData(payload) {
		...
		if (void 0 !== payload.data) return payload.data;
		const { message: ignoredMessage } = payload, rest = _objectWithoutProperties(payload, _excluded);
	...
	function buildAgentToolResponse(response, state) {
		...
		const payload = (_response$data = response === null || response === void 0 ? void 0 : response.data) !== null && _response$data !== void 0 ? _response$data : null;
		const message = (_ref = (_payload$message = payload === null || payload === void 0 ? void 0 : payload.message) !== null && _payload$message !== void 0 ? _payload$message : response === null || response === void 0 ? void 0 : response.message) !== null && _ref !== void 0 ? _ref : "";
		return {
	...
	function respondToAgentIfInvoked(event, response, state) {
		if (!(event === null || event === void 0 ? void 0 : event.agentInvoked) || "function" !== typeof event.respondWith) return;
		event.respondWith(Promise.resolve(buildAgentToolResponse(response, state)));
	}
	function registerAtomicFormAlpineData(form) {
		var _alpinejs$Alpine;
		if (!form || !((_alpinejs$Alpine = _alpinejs.Alpine) === null || _alpinejs$Alpine === void 0 ? void 0 : _alpinejs$Alpine.data)) return;
		const alpineId = (0, _utils.getAlpineId)(form);
		if (!alpineId) return;
		_alpinejs.Alpine.data(alpineId, () => ({ submit(event) {
			return _asyncToGenerator(function* () {
				if ((0, _utils.isEditorContext)()) return;
				...
				try {
					const response = yield submitAtomicForm(payload);
					const state = (response === null || response === void 0 ? void 0 : response.success) ? "success" : "error";
					...
					respondToAgentIfInvoked(event, {
						success: false,
						message: (_error$message = error === null || error === void 0 ? void 0 : error.message) !== null && _error$message !== void 0 ? _error$message : "Submission failed."
					}, "error");
				} finally {
					clearAtomicFormSubmittingState(form, submitButtons);
				}
	...
	function buildAtomicFormPayload(form) {
		...
		if (!postId || !formFields.length) return null;
		return {
			postId,
			formId,
			formName,
	...
	function getGroupedFieldId(name, inputs, checkedInputs) {
		var _checkedInputs$;
		var _primaryInput$dataset;
		const primaryInput = (_checkedInputs$ = checkedInputs[0]) !== null && _checkedInputs$ !== void 0 ? _checkedInputs$ : inputs[0];
		return (_primaryInput$dataset = primaryInput === null || primaryInput === void 0 ? void 0 : primaryInput.dataset.interactionId) !== null && _primaryInput$dataset !== void 0 ? _primaryInput$dataset : name;
	}
	...
	function getAtomicFieldOptions(fieldOrGroup, type, form) {
		...
		if (Array.isArray(fieldOrGroup) && ["radio", "checkbox"].includes(type)) return fieldOrGroup.map((input) => {
			return {
				value: input.value,
	...
	function getAtomicFormFieldLabel(field, form) {
		...
		if (fieldId) {
			var _labelElement$textCon;
			const labelElement = form.querySelector(`label[for="${fieldId}"]`);
			const labelText = labelElement === null || labelElement === void 0 || (_labelElement$textCon = labelElement.textContent) === null || _labelElement$textCon === void 0 ? void 0 : _labelElement$textCon.trim();
			if (labelText) return labelText;
		}
		const parentLabelText = (_field$closest = field.closest("label")) === null || _field$closest === void 0 || (_field$closest = _field$closest.textContent) === null || _field$closest === void 0 ? void 0 : _field$closest.trim();
		if (parentLabelText) return parentLabelText;
		return field.getAttribute("placeholder") || fieldId || "";
	}
	function getAtomicFormFieldValue(inputs, type) {
		if ("file" === type) return inputs.files;
		if (Array.isArray(inputs)) return inputs.map((input) => input.value).join(", ");
		if ("checkbox" === type) return inputs.checked ? inputs.value || "on" : "";
		return inputs.value;
	}
	function getAtomicFormFieldType(field) {
		const tagName = field.tagName.toLowerCase();
		if ("input" === tagName) return field.getAttribute("type") || "text";
		return tagName;
	}
	...
	function submitAtomicForm(_x) {
		return _submitAtomicForm.apply(this, arguments);
	}
	function _submitAtomicForm() {
		_submitAtomicForm = _asyncToGenerator(function* (payload) {
			...
			var _window;
			const ajaxUrl = (_elementorFrontendCon = elementorFrontendConfig) === null || _elementorFrontendCon === void 0 || (_elementorFrontendCon = _elementorFrontendCon.urls) === null || _elementorFrontendCon === void 0 ? void 0 : _elementorFrontendCon.ajaxurl;
			if (!ajaxUrl) return { success: false };
			const formData = new FormData();
			formData.append("action", "elementor_pro_atomic_forms_send_form");
			formData.append("_nonce", (_elementorFrontendCon2 = elementorFrontendConfig) === null || _elementorFrontendCon2 === void 0 || (_elementorFrontendCon2 = _elementorFrontendCon2.nonces) === null || _elementorFrontendCon2 === void 0 ? void 0 : _elementorFrontendCon2.atomicFormsSendForm);
			formData.append("post_id", payload.postId);
			formData.append("form_id", payload.formId);
			formData.append("form_name", payload.formName);
			formData.append("referer_title", (_document$title = (_document = document) === null || _document === void 0 ? void 0 : _document.title) !== null && _document$title !== void 0 ? _document$title : "");
			formData.append("referrer", (_window$location$href = (_window = window) === null || _window === void 0 || (_window = _window.location) === null || _window === void 0 ? void 0 : _window.href) !== null && _window$location$href !== void 0 ? _window$location$href : "");
	...
	function setFocusOnMessageElement(element, type) {
		...
		messageElement === null || messageElement === void 0 || messageElement.focus({ focusVisible: true });
	}
