...
//#endregion
//#region assets/dev/js/frontend/handlers/video.js
	var require_video = /* @__PURE__ */ __commonJSMin(((exports) => {
		...
		var Video = class extends elementorModules.frontend.handlers.Base {
			getDefaultSettings() {
				return { selectors: {
			...
			handleVideo() {
				if (this.getElementSettings("lightbox")) return;
				if ("youtube" === this.getElementSettings("video_type")) this.apiProvider.onApiReady((apiObject) => {
			...
			playVideo() {
				if (this.elements.$video.length) {
					...
					else this.elements.$video[0].play();
					return;
				}
				const $videoIframe = this.elements.$videoIframe;
				const lazyLoad = $videoIframe.data("lazy-load");
				if (lazyLoad) $videoIframe.attr("src", lazyLoad);
			...
			animateVideo() {
				...
				return _asyncToGenerator(function* () {
					(yield elementorFrontend.utils.lightbox).setEntranceAnimation(_this.getCurrentDeviceSetting("lightbox_content_animation"));
				})();
			}
			hideLightbox() {
				return _asyncToGenerator(function* () {
					(yield elementorFrontend.utils.lightbox).getModal().hide();
				})();
			}
			prepareYTVideo(YT, onOverlayClick) {
				...
				const playerOptions = {
					...
					events: {
						onReady: () => {
							if (elementSettings.mute) this.youtubePlayer.mute();
							if (elementSettings.autoplay || onOverlayClick) this.youtubePlayer.playVideo();
						},
						onStateChange: (event) => {
							if (event.data === YT.PlayerState.ENDED && elementSettings.loop) this.youtubePlayer.seekTo(elementSettings.start || 0);
						}
					},
					playerVars: {
			...
			bindEvents() {
				...
				this.elements.$playIcon.on("keydown", (event) => {
					if ([13, 32].includes(event.keyCode)) this.handleVideo();
				});
			}
			onInit() {
				super.onInit();
				const elementSettings = this.getElementSettings();
				if (elementorFrontend.utils[elementSettings.video_type]) this.apiProvider = elementorFrontend.utils[elementSettings.video_type];
				else this.apiProvider = elementorFrontend.utils.baseVideoLoader;
			...
			onElementChange(propertyName) {
				...
				if ("lightbox" === propertyName && !isLightBoxEnabled) this.hideLightbox();
			}
		};
...
//#region \0elementor-chunk-entry
	var import_video = /* @__PURE__ */ __toESM(require_video());
	(window.__elementorChunks = window.__elementorChunks || {})["video"] = import_video;
