...
//#endregion
//#region assets/dev/js/frontend/handlers/background-video.js
	var require_background_video = /* @__PURE__ */ __commonJSMin(((exports) => {
		Object.defineProperty(exports, "__esModule", { value: true });
		exports.default = void 0;
		require_esnext_iterator_constructor();
		require_esnext_iterator_find();
		var BackgroundVideo = class extends elementorModules.frontend.handlers.Base {
			getDefaultSettings() {
				return { selectors: {
			...
			getDefaultElements() {
				...
				const elements = { $backgroundVideoContainer: this.$element.find(selectors.backgroundVideoContainer) };
			...
			calcVideosSize($video) {
				...
				if ("vimeo" === this.videoType) aspectRatioSetting = $video[0].width + ":" + $video[0].height;
			...
			changeVideoSize() {
				...
				let $video;
				if ("youtube" === this.videoType) $video = jQuery(this.player.getIframe());
				else if ("vimeo" === this.videoType) $video = jQuery(this.player.element);
				else if ("hosted" === this.videoType) $video = this.elements.$backgroundVideoHosted;
				if (!$video) return;
			...
			prepareVimeoVideo(Vimeo, videoLink) {
				...
				if (elementSettings.background_privacy_mode) vimeoOptions.dnt = true;
			...
			handleVimeoStartEndTimes(elementSettings) {
				if (elementSettings.background_video_start) this.player.on("play", (data) => {
					if (0 === data.seconds) this.player.setCurrentTime(elementSettings.background_video_start);
				});
				this.player.on("timeupdate", (data) => {
					if (elementSettings.background_video_end && elementSettings.background_video_end < data.seconds) if (elementSettings.background_play_once) this.player.pause();
					else this.player.setCurrentTime(elementSettings.background_video_start);
					this.player.getDuration().then((duration) => {
						if (elementSettings.background_video_start && !elementSettings.background_video_end && data.seconds > duration - .5) this.player.setCurrentTime(elementSettings.background_video_start);
					});
				});
			}
						...
						onStateChange: (event) => {
							switch (event.data) {
								...
								case YT.PlayerState.ENDED:
									if ("function" === typeof this.player.seekTo) this.player.seekTo(elementSettings.background_video_start || 0);
									if (elementSettings.background_play_once) this.player.destroy();
							}
						}
					},
					playerVars: {
			...
			activate() {
				...
				if (this.apiProvider) {
					...
					this.apiProvider.onApiReady((apiObject) => {
						if ("youtube" === this.videoType) this.prepareYTVideo(apiObject, videoID);
						if ("vimeo" === this.videoType) this.prepareVimeoVideo(apiObject, videoLink);
					});
				} else {
					...
					if (playOnce) this.elements.$backgroundVideoHosted.on("ended", () => {
			...
			deactivate() {
				if ("youtube" === this.videoType && this.player.getIframe() || "vimeo" === this.videoType) this.player.destroy();
				else this.elements.$backgroundVideoHosted.removeAttr("src").off("ended");
				elementorFrontend.elements.$window.off("resize", this.changeVideoSize);
			}
			run() {
				const elementSettings = this.getElementSettings();
				if (!elementSettings.background_play_on_mobile && "mobile" === elementorFrontend.getCurrentDeviceMode()) return;
				if ("video" === elementSettings.background_background && elementSettings.background_video_link) this.activate();
			...
			onElementChange(propertyName) {
				if ("background_background" === propertyName) this.run();
			}
		};
...
//#region \0elementor-chunk-entry
	var import_background_video = /* @__PURE__ */ __toESM(require_background_video());
	(window.__elementorChunks = window.__elementorChunks || {})["background-video"] = import_background_video;
