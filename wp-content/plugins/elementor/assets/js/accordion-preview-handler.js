...
//#endregion
//#region modules/atomic-widgets/elements/atomic-accordion/handlers/editor-accordion-state.js
	...
	var ITEM_ELEMENT_TYPE = "e-accordion-item";
	var HEADER_ELEMENT_TYPE = "e-accordion-item-header";
	var MAX_EXPANDED_ONE = "one";
	var DEFAULT_STATE_FIRST_EXPANDED = "first_expanded";
	var SELECT_COMMAND = "document/elements/select";
	var COMMAND_RUN_AFTER_EVENT = "elementor/commands/run/after";
	/**
	...
	*/
	(0, _frontendHandlers.register)({
		...
		callback: ({ element, signal, listenToChildren }) => {
			...
			const getItems = () => Array.from(element.querySelectorAll(`[data-element_type="${ITEM_ELEMENT_TYPE}"]`)).filter(belongsToThisAccordion);
			const getRootSetting = (key, fallback) => {
				var _window$parent;
				var _window$parent$getCon;
				var _container$settings;
				var _container$settings$g;
				var _settingValue$origin_;
				const container = (_window$parent = window.parent) === null || _window$parent === void 0 || (_window$parent = _window$parent.elementor) === null || _window$parent === void 0 || (_window$parent$getCon = _window$parent.getContainer) === null || _window$parent$getCon === void 0 ? void 0 : _window$parent$getCon.call(_window$parent, element.dataset.id);
				const setting = container === null || container === void 0 || (_container$settings = container.settings) === null || _container$settings === void 0 || (_container$settings$g = _container$settings.get) === null || _container$settings$g === void 0 ? void 0 : _container$settings$g.call(_container$settings, key);
				const settingValue = setting && "object" === typeof setting && "value" in setting ? setting.value : setting;
				const value = settingValue && "object" === typeof settingValue ? (_settingValue$origin_ = settingValue.origin_value) === null || _settingValue$origin_ === void 0 ? void 0 : _settingValue$origin_.value : settingValue;
			...
			const openItem = (item) => {
				if (isExclusive()) getItems().forEach((sibling) => {
					if (sibling !== item) sibling.removeAttribute("open");
				});
				item.setAttribute("open", "");
			};
			let userToggledAnItem = false;
			const defaultStateEvaluated = /* @__PURE__ */ new WeakSet();
			/**
			...
			*/
			const applyDefaultState = () => {
				...
				items.forEach((item, index) => {
					if (defaultStateEvaluated.has(item)) return;
					defaultStateEvaluated.add(item);
					if (shouldExpandFirst && 0 === index && !hasOpenItem) item.setAttribute("open", "");
				});
			};
			applyDefaultState();
			listenToChildren([ITEM_ELEMENT_TYPE]).render((event) => {
				if (belongsToThisAccordion(event.detail.element)) applyDefaultState();
			});
			let itemToggledByClick = null;
			element.addEventListener("click", (event) => {
				var _event$target;
				var _event$target$closest;
				const header = (_event$target = event.target) === null || _event$target === void 0 || (_event$target$closest = _event$target.closest) === null || _event$target$closest === void 0 ? void 0 : _event$target$closest.call(_event$target, `[data-element_type="${HEADER_ELEMENT_TYPE}"]`);
				const item = header === null || header === void 0 ? void 0 : header.closest(`[data-element_type="${ITEM_ELEMENT_TYPE}"]`);
				if (!item || !belongsToThisAccordion(item)) return;
			...
			*/
			window.parent.addEventListener(COMMAND_RUN_AFTER_EVENT, (event) => {
				var _event$detail;
				var _window$parent$elemen;
				var _window$parent2;
				var _window$parent2$getEl;
				var _selected$model;
				var _selected$model$get;
				if (((_event$detail = event.detail) === null || _event$detail === void 0 ? void 0 : _event$detail.command) !== SELECT_COMMAND) return;
				const [selected] = (_window$parent$elemen = (_window$parent2 = window.parent) === null || _window$parent2 === void 0 || (_window$parent2 = _window$parent2.elementor) === null || _window$parent2 === void 0 || (_window$parent2 = _window$parent2.selection) === null || _window$parent2 === void 0 || (_window$parent2$getEl = _window$parent2.getElements) === null || _window$parent2$getEl === void 0 ? void 0 : _window$parent2$getEl.call(_window$parent2)) !== null && _window$parent$elemen !== void 0 ? _window$parent$elemen : [];
				const selectedId = selected === null || selected === void 0 || (_selected$model = selected.model) === null || _selected$model === void 0 || (_selected$model$get = _selected$model.get) === null || _selected$model$get === void 0 ? void 0 : _selected$model$get.call(_selected$model, "id");
				...
				if (!item || !belongsToThisAccordion(item) || item === itemToggledByClick) return;
...
//# sourceMappingURL=accordion-preview-handler.js.map