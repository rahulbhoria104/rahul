...
//#endregion
//#region assets/dev/js/admin/hints/media.js
	...
	(function(_elementorAdminHints, _wp) {
		if (!((_elementorAdminHints = elementorAdminHints) !== null && _elementorAdminHints !== void 0 && _elementorAdminHints.mediaHint)) return;
		if (!((_wp = wp) !== null && _wp !== void 0 && (_wp = _wp.media) !== null && _wp !== void 0 && (_wp = _wp.view) !== null && _wp !== void 0 && (_wp = _wp.Attachment) !== null && _wp !== void 0 && _wp.Details)) return;
		wp.media.view.Attachment.Details = wp.media.view.Attachment.Details.extend({
			...
			template: function template(view) {
				var html = wp.media.template("attachment-details")(view);
				var dom = document.createElement("div");
				dom.innerHTML = html;
				if (!this.shouldDisplayHint(view)) return dom.innerHTML;
				var hint = document.createElement("div");
				hint.classList.add("e-hint");
				hint.innerHTML = _.template(this._tmpl)(elementorAdminHints.mediaHint);
				dom.querySelector(".attachment-info").appendChild(hint);
			...
			shouldDisplayHint: function shouldDisplayHint(view) {
				...
				if (!elementorAdminHints || !((_elementorAdminHints2 = elementorAdminHints) !== null && _elementorAdminHints2 !== void 0 && _elementorAdminHints2.mediaHint)) return false;
				if (window.elementorHints !== void 0) return false;
				if (view.type !== "image") return false;
				if (elementorAdminHints.mediaHint.display) return true;
			...
			onHintAction: function onHintAction(event) {
				...
				var _JSON$parse$action_ur = JSON.parse(settings).action_url;
			...
			dismiss: function dismiss(event) {
				elementorCommon.ajax.addRequest("dismissed_editor_notices", { data: { dismissId: event.target.closest(".e-hint__container").dataset.event } });
				this.hideHint(event);
			},
