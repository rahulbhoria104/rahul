...
//#region modules/interactions/assets/js/interactions-shared-utils.js
	...
	function preserveTransformKeyframes(keyframes, baseline) {
		...
		var mergedKeyframes = _objectSpread$1({}, keyframes);
...
//#region modules/interactions/assets/js/interactions-utils.js
	function getKeyframes(effect, type, direction) {
		...
		var config$1 = config();
		if ("scale" === effect) keyframes.scale = isIn ? [config$1.scaleStart, 1] : [1, config$1.scaleStart];
		if (direction && "string" === typeof direction) {
			var distance = config$1.slideDistance;
			var movement = {
	...
	function parseAnimationName(name) {
		var _name$split2 = _slicedToArray(name.split("-"), 8);
		var trigger = _name$split2[0];
		var effect = _name$split2[1];
		var type = _name$split2[2];
		var direction = _name$split2[3];
		var duration = _name$split2[4];
		var delay = _name$split2[5];
		var config$2 = config();
		return {
			trigger,
			effect,
			...
			duration: duration ? parseInt(duration, 10) : config$2.defaultDuration,
			delay: delay ? parseInt(delay, 10) : config$2.defaultDelay,
			replay: false,
			easing: config$2.defaultEasing
		};
	}
	function unwrapInteractionBreakpoints(propValue) {
		var breakpointsConfig = unwrapInteractionValue(propValue, {});
		var excluded = unwrapInteractionValue(breakpointsConfig === null || breakpointsConfig === void 0 ? void 0 : breakpointsConfig.excluded, []);
		if (1 > excluded.length) return {};
		return { excluded: excluded.map(function(breakpoint) {
			return unwrapInteractionValue(breakpoint, "");
		}) };
	}
	function extractAnimationConfig(interaction) {
		...
		if (payload !== null && payload !== void 0 && (_payload$animation = payload.animation) !== null && _payload$animation !== void 0 && _payload$animation.animation_id) return parseAnimationName(payload.animation.animation_id);
		var trigger = unwrapInteractionValue(payload.trigger) || payload.trigger || "load";
		...
		var config$3 = config();
		var effect = unwrapInteractionValue(animation.effect) || animation.effect || "fade";
		var type = unwrapInteractionValue(animation.type) || animation.type || "in";
		var directionUnwrapped = unwrapInteractionValue(animation.direction);
		var direction = "string" === typeof directionUnwrapped ? directionUnwrapped : "";
		var easing = config$3.defaultEasing;
		var replay = false;
		var timingConfig = unwrapInteractionValue(animation.timing_config) || animation.timing_config || {};
		return {
			trigger,
			breakpoints,
			effect,
			type,
			direction,
			duration: timingValueToMs(timingConfig === null || timingConfig === void 0 ? void 0 : timingConfig.duration, config$3.defaultDuration),
			delay: timingValueToMs(timingConfig === null || timingConfig === void 0 ? void 0 : timingConfig.delay, config$3.defaultDelay),
			easing,
			replay
		};
	}
...
//#endregion
//#region modules/interactions/assets/js/editor-interactions.js
	...
	*/
	...
	function applyAnimation(element, animConfig, animateFunc) {
		...
		var baseline = getTransformBaselineFromComputedStyle(element);
		var keyframes = preserveTransformKeyframes(getKeyframes(animConfig.effect, animConfig.type, animConfig.direction), baseline);
		var options = {
			duration: animConfig.duration / 1e3,
			delay: animConfig.delay / 1e3,
		...
		animateFunc(element, initialKeyframes, { duration: 0 }).then(function() {
	...
	function applyInteractionsToElement(element, interactionsData) {
		var animateFunc = getAnimateFunction();
		if (!animateFunc) return;
		var parsedData = parseInteractionsData(interactionsData);
		if (!parsedData) return;
		Object.values((parsedData === null || parsedData === void 0 ? void 0 : parsedData.items) || []).forEach(function(interaction) {
			var animConfig = extractAnimationConfig(interaction);
	...
	function handleInteractionsUpdate() {
		...
		currentInteractionsData.filter(function(currentItem) {
			var _currentItem$interact;
			var _previousItem$interac;
			var previousItem = previousInteractionsData.find(function(prev) {
				return prev.dataId === currentItem.dataId;
			});
			if (!previousItem) return true;
			return (((_currentItem$interact = currentItem.interactions) === null || _currentItem$interact === void 0 ? void 0 : _currentItem$interact.items) || []).map(extractInteractionId).filter(Boolean).sort().join(",") !== (((_previousItem$interac = previousItem.interactions) === null || _previousItem$interac === void 0 ? void 0 : _previousItem$interac.items) || []).map(extractInteractionId).filter(Boolean).sort().join(",");
		}).forEach(function(item) {
			var _previousInteractions;
			var _item$interactions;
			...
			if (!element || !((_item$interactions = item.interactions) !== null && _item$interactions !== void 0 && (_item$interactions = _item$interactions.items) !== null && _item$interactions !== void 0 && _item$interactions.length)) return;
			var prevIds = new Set(((prevInteractions === null || prevInteractions === void 0 ? void 0 : prevInteractions.items) || []).map(extractInteractionId).filter(Boolean));
			var changedInteractions = item.interactions.items.filter(function(interaction) {
				var id = extractInteractionId(interaction);
				return !id || !prevIds.has(id);
			});
			if (changedInteractions.length > 0) applyInteractionsToElement(element, _objectSpread(_objectSpread({}, item.interactions), {}, { items: changedInteractions }));
		});
		previousInteractionsData = currentInteractionsData;
	}
	function initEditorInteractionsHandler() {
		waitForAnimateFunction(function() {
	...
	function handlePlayInteractions(event) {
		...
		var elementId = _event$detail.elementId;
		var interactionId = _event$detail.interactionId;
		var item = getInteractionsData().find(function(elementItemData) {
			return elementItemData.dataId === elementId;
		});
		if (!item) return;
		var element = findElementByInteractionId(elementId);
		if (!element) return;
		applyInteractionsToElement(element, _objectSpread(_objectSpread({}, item.interactions), {}, { items: item.interactions.items.filter(function(interactionItem) {
