...
//#region modules/interactions/assets/js/interactions-breakpoints.js
	...
	function initBreakpoints() {
		var onChange = (arguments.length > 0 && arguments[0] !== void 0 ? arguments[0] : {}).onChange;
...
//#region modules/interactions/assets/js/interactions-shared-utils.js
	...
	function skipInteraction$1(interaction) {
	...
	__name(skipInteraction$1, "skipInteraction");
	...
	window.elementorModules.interactions = {
		config,
		skipInteraction: skipInteraction$1,
...
//#region modules/interactions/assets/js/interactions-utils.js
	function isSupportedInteraction(animationConfig) {
		if (![
			...
			"scrollIn",
			"scrollOut"
		].includes(animationConfig.trigger)) return false;
		if ("custom" === animationConfig.effect) return false;
		return true;
	}
	function skipInteraction(animationConfig) {
		if (!isSupportedInteraction(animationConfig)) return true;
		return skipInteraction$1(animationConfig);
	}
...
//#endregion
//#region modules/interactions/assets/js/interactions.js
	function scrollOutAnimation(element, transition, animConfig, keyframes, resetKeyframes, options, animateFunc, inViewFunc) {
		...
		var stop = inViewFunc(element, function() {
			return function() {
				...
				if (false === animConfig.replay) Promise.resolve().then(function() {
					return stop();
				});
			};
		}, viewOptions);
	...
	function scrollInAnimation(element, transition, animConfig, keyframes, options, animateFunc, inViewFunc) {
		var stop = inViewFunc(element, function() {
			...
			if (false === animConfig.replay) stop();
		}, {
			amount: 0,
			root: null
		});
	...
	function applyAnimation(element, animConfig, animateFunc, inViewFunc) {
		...
		var resetKeyframes = preserveTransformKeyframes(getKeyframes(animConfig.effect, "in", animConfig.direction), baseline);
		var options = {
		...
		if ("scrollOut" === animConfig.trigger) scrollOutAnimation(element, transition, animConfig, keyframes, resetKeyframes, options, animateFunc, inViewFunc);
	...
	function processElementInteractions(element, interactions, animateFunc, inViewFunc) {
		...
		interactions.forEach(function(interaction) {
			...
			if (animConfig && !skipInteraction(animConfig)) applyAnimation(element, animConfig, animateFunc, inViewFunc);
		});
	}
	function initInteractions() {
		waitForAnimateFunction(function() {
			var animateFunc = getAnimateFunction();
			var inViewFunc = getInViewFunction();
			if (!inViewFunc || !animateFunc) return;
			var dataScript = document.getElementById("elementor-interactions-data");
			if (dataScript) {
				JSON.parse(dataScript.textContent).forEach(function(elementData) {
					var elementId = elementData.elementId;
					var interactions = elementData.interactions;
			...
			document.querySelectorAll("[data-interactions]").forEach(function(element) {
				processElementInteractions(element, parseInteractionsData(element.getAttribute("data-interactions")), animateFunc, inViewFunc);
			});
		});
	}
	function init() {
